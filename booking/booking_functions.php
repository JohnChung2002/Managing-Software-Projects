...
function newenquiry(){
    if (!empty($_POST["name"]) && !empty($_POST["email"])  && !empty($_POST["InputReason"]) && !empty($_POST["inputComment"])) {
        ...
        while (True){
            // $enquiry_id = $_POST["SELECT enquiry_id FROM enquiries"];

            $command = "SELECT * FROM enquiries;";
            $stmt = mysqli_prepare($conn, $command);
            ...
            if (mysqli_num_rows($result) == 0) {
                mysqli_free_result($result);
                $command = "INSERT INTO enquiries (contact_name, contact_info, enquiry_subject, enquiry_content, enquiries_status) VALUES (?, ?, ?, ?, 'Unanswered');";
                $stmt = mysqli_prepare($conn, $command);
                mysqli_stmt_bind_param($stmt, "ssss", $name, $email, $InputReason, $InputComment);
                if (mysqli_stmt_execute($stmt)) {
                    echo "
                    <div class='container'>
                        <div class='alert alert-success mt-4'>
...
function getEnquiryRequest(){
    $conn = start_connection();
    $test = "SELECT enquiry_id FROM enquiries;";
    $enquiry_id = $_GET[$test];

    // $enquiry_id = $_POST["enquiry_id"];
    // "SELECT * FROM `enquiries` ORDER BY `enquiry_id`  DESC;";
